...
func _find_closest_player() -> void:
	...
	for player in _players_node.get_children():
		# Skip locked up players and immune players
		if player.has_method("set_locked_up") and player.is_locked_up:
			continue
		# Skip knocked out or immune players for targeting
		if player.has_method("set_knocked_out") and (player.is_knocked_out or player.is_knockout_immune):
			continue
