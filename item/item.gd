...
@export var max_carry_distance: float = 100.0  # Maximum distance between carriers
...
func _physics_process(delta: float) -> void:
    # Check distance between carriers for two-carrier items
    if requires_two_carriers and carrier_ids.size() == 2 and multiplayer.is_server():
        if not check_carrier_distance():
            # Players are too far apart, drop the item
            print("Carriers too far apart, dropping item")
            force_drop_item()
            return
...
# Check if carriers are within acceptable distance
func check_carrier_distance() -> bool:
    if carrier_ids.size() != 2:
        return true  # No distance check needed
    
    var players_node = get_tree().get_root().get_node("Demo1/Players")
    var player1 = players_node.get_node(str(carrier_ids[0]))
    var player2 = players_node.get_node(str(carrier_ids[1]))
    ...
    if player1 and player2:
        ...
        return distance <= max_carry_distance
    
    return true  # If we can't find players, don't drop

# Force drop the item when carriers are too far apart
func force_drop_item() -> void:
    print("Force dropping item due to distance")
    isCarried = false
    drop_item_at_center()
    ...
    # Notify all clients about the drop
    item_force_dropped.rpc()

@rpc("authority", "call_local")
func item_force_dropped() -> void:
    print("Item force dropped due to distance between carriers")
    isCarried = false
    carrier_ids.clear()
